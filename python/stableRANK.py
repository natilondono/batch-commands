    ...
    def plot(self, style="bar"):
        if isinstance(self.input,dict):
            if style=="bar":
                ...
                for key in  self.input:
                    if np.isfinite(self.input[key]).all():
                        ...
                        plt.hlines(y,self.birth[key],self.death[key],linewidth=3)
            ...
            if style=="bar":
                ...
                plt.hlines(y,self.birth,self.death,linewidth=3,color='red')
            else:
                return print("for now only bar style")
